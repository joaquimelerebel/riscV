...
int __ptr_overflow_check__(void* ptr, size_t size) {
	// printk("__ptr_overflow_check__: called at %x with size %d\n", ptr, size);
	if ( size && ptr >= ((char *)HEAP_BASE) ) {
		// printk("__ptr_overflow_check__: ptr in heap\n");
		if ( ptr > ((char *)HEAP_BASE) + heap_sz) {
			// printk("__ptr_overflow_check__: size heap\n");
			return -1;
		}
		
		return __heap_overflow_check__(ptr, size);
	} else {
		// printf("__ptr_overflow_check__: outside heap\n");
	}
