...
void z_riscv_switch_nx_thread_stack(struct k_thread *thread) {
	struct _thread_stack_info stack_info = thread->stack_info;
	uint32_t start  = stack_info.start;
	uint32_t end = start + stack_info.size + stack_info.delta;
	csr_write(CSR_NX_START, start);
	csr_write(CSR_NX_END, end);
}
#endif /* CONFIG_CUSTOM_PROT */